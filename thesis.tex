...
When counter value becomes $0$ then we move to some regognized state $R_i$ (notice that for
correct representations of stacks we never get remainder $0$, so we never end within $q_0$).
...
we need to copy it back to original counter.
