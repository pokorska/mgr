...
Solutions \texttt{to\_lower.bf} and \texttt{add\_digits.bf} perform basic
computations and it is possible to run them in $4$CM, which is most of the pipeline.

There is another group of solutions \texttt{simple.tm} and \texttt{rotate.tm}
which does not start from Brainfuck code. For them, it is easier to express
the logic in Turing Machine code. Rotation is implementing Ceasar cypher
and it still runs within less than $1$s after translating to $4$CM.
...
require transformation, so output counter contains only $65$,
no power and no exponential growth.


Whole evaluation was performed on Intel(R) Core(TM) i7-5600U 2.60GHz
