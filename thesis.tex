...
(we cannot move left or right explicitly). Similarly, \textit{output tape} is write-only and
...
or~\textit{reject} original input word and there may be an~output word written on the output tape
...
then we get an equivalent model, the~same for $4$ or~more counters. According to~this fact, we~can
...
($a$,~$s$,~$t$)~$\in$~($\Sigma$,~$\Gamma$,~$\Gamma$). It means that the input symbol is $a$,
...
The~flexibility in running the~pipeline gives us the possibility to prove (as~side-effect)
...
from special definitions that are allowed to appear as \texttt{symbol\_to\_write}
...
Note: If there is no state change defined for the given configuration
...
matching all counters \textbf{the first one} is applied. It means that the order
...
there are a separate source and a separate destination for I/O operations.
...
we cut the~tape into two pieces in a~way that the current cell is at the~top of the~left stack and all
...
need to copy the~result from helper counter into the~original one. The whole operation is shown in
...
the recognized symbol has number $i$. Consider example alphabet having $9$ symbols -- Figure
...
\caption{Recognizing symbol from an alphabet of size $9$. $R_i$ is the~state with recognized
...
to copy it into our counter for computations and after recognizing symbol at the~top
...
\textbf{check} operations. Because both symbols need to match, after recognizing the first symbol
we need to recognize the other without losing the information about the~first one. This
...
holding state and the other was used for calculations. We will do a similar
...
(exponential) and tahe way this (exponential) counter is used, it is really
...
The cruelness of the ASCII table can be defeated though. The only thing we need
is to operate on small numbers in memory, while big numbers (or rather characters
assigned to them) are printed to the standard output. This leads us to shifting
printed character by $64$ with newly created operator \texttt{:} (semicolon) which is
very similar to Brainfuck's \texttt{.} (dot) printing a character without any shifts. Example:
\begin{verbatim}
+++:++:++:
\end{verbatim}
This code will print \texttt{CEG} on standard output.

Obviously this modification does not change the power of expression
of that language, because it is actually a sintactic sugar and may be translated
...
increase (by $64$) the number pushed into output counter. After the translation
...
\begin{itemize}
  ...
  \item \texttt{-o <output file path>} or \texttt{--output <output file path>} -- specifies
      the path to output directory (result of translation) written in required language (2 Counter Machine by default).
  ...
  \item \texttt{-d} or \texttt{--direct} directly runs translated code. If input
      ...
      part, for example files \texttt{outdir/aaa\_123}, \texttt{outdir/aaa\_43} should
      be provided as a command \texttt{./full\_pipeline -d outdir/aaa}
