...
It has all adventages and~disadventages which belong to~modern programs, i.e., it~is possible
...
work, it has access to three different tapes: \textit{input tape}, \textit{output tape} and
...
a~\textbf{run} of~a~Turing Machine. This sequence may be finite or~infinite in the~same way like
...
of~given algorithm using a~modern programming language. From the thesis we~claim that
...
integer, but the~machine is~not allowed to~explicitly check its value, it~just receives a~boolean
information if it is equal to $0$.

There are few alternative models -- if~we~allow the~machine to~use at~least two counters,
...
Of~course halting problem for Two Counter Machine is proven to~be not decidable. \cite{minsky_67}
...
is critical to know what is the order of intructions in Brainfuck.
...
explains how we approach each separate translation -- the general algorithm to~move from the~source code in one model
to the~source code in the~next one. Section 4 contains technical details about the~implementation -- which tools
were chosen for~this project and how to use the~prepared code. It also discusses encountered problems and presents
...
It turns out that Brainfuck has the very specific instructions that allow making optimizations
...
\begin{itemize}
  ...
  \item 2 stack PDA interpreter always recognizes symbols on both stacks
      ...
      increases it by $1$ and stores it back onto the same (left) stack.
      This means that the right stack may be completely ignored. This will
      result in O($m$) transitions in $4$CM from a single $2$ stack PDA transition.
\end{itemize}
...
\item There is no need to load all the states at the beginning, because most probably we will
  ...
  \item In $2$CM transitions we often need to recover counter's position by
      ...
      $1$ and adds $1$ on desired counters, which is very ineffective copy.
      Such a transition is recognized and the copy is done immedietaly with
  ...
  \item \texttt{2stackPDA\_parser}
    \begin{itemize}
      ...
      \item (not recommended) translate 2 stack PDA to 4 Counter Machine: \\ \texttt{./test.e -minsky ../examples/hello.pda}
        ...
        which is usually very slow. Instead I recommend to use multifile
...
and it runs within less than $1$s after translating to $4$CM. \texttt{simple.tm}
