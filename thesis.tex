...
We can clearly see that the problems appear starting at $4$CM, where
...
\begin{itemize}
  \item \texttt{hello.bf} -- stores few things in different cells
      (separately characters near \texttt{H} and whitespaces),
  \item \texttt{hello3.bf} -- uses only one cell to store all letters to print,
  \item \texttt{hello4.xbf} -- uses only one cell and operates on small numbers
      within this cell using shifted print.
\end{itemize}

They contain optimizations for latest stages of translations. \texttt{hello3.bf}
is running smothly up to $4$CM, while \texttt{hello4.xbf} is runnig smothly
throughout the whole pipeline. The main difference is using the recourses,
so $4$CM needs usage of only $1$ memory cell in original Brainfuck's code,
while $2$CM needs additionally small numbers stored within this cell.

Porównanie czasów, rozmiarów (na przykładach, nie musi być ,,zupełne'')
\todo{Dopisać słowną analizę tabelki poniżej.}

Whole evaluation was performed on a machine with Intel(R) Core(TM) i7-5600U 2.60GHz
with $4$ cores. Operating system that was used: Linux thinkpad 4.15.0-45-generic \#48-Ubuntu.
...
\texttt{to\_lower.bf} & \# & 34 & 35 & 36 & 0.4 M & 4.3 M\\
 ...
 & {\footnotesize{TT}} & 0.00 s & 0.01 s & 21.40 s & 43.21 s & --- \\
\hline
...
\texttt{simple.tm} & \# & --- & 8 & 11 & 0.3 M & 3.9 M\\
 ...
 & {\footnotesize{TT}} & --- & 0.00 s & 14.57 s & 38.02 s & --- \\
\hline
