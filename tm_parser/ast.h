...
class TransitionMap {
 ...
 public:
  ...
  std::string buildTapeExtension(const TransitionRaw& t) {
    ...
    std::string output_char_section = (t.type == TransitionRaw::Output) ? " ORIG_LEFT" : "";
    result += t.curr_state + " " + t.pattern + " " + EMPTY_STACK_CHAR + " " + input_char_section +
              transitionTypeToString(t.type) + " (" + input_handling + " + BLANK) " +
  ...
  std::string translate() {
    ...
    for (const auto& alt : transitions) {
      for (const TransitionRaw& t : alt.second) {
        ...
        result += t.curr_state + " " + t.pattern + " " + ALL_CHARS + " " + input_char_section +
                  transitionTypeToString(t.type) + " " + left_stack + " " + right_stack +
